...
/**
 * Minimal inline trend line — no chart library, no extra deps.
 * Respects prefers-reduced-motion by simply not animating (duration=0 skips the clip-reveal).
 */
export const Sparkline = memo(function Sparkline({
  ...
  // Guard against the two shapes that break getSparkCoordinates: no points, and a
  // single point (division by zero in the min/max range and the x-position denominator).
  ...
  const points = useMemo(
    () => (hasEnoughData ? getSparkCoordinates({ data, height, width }) : []),
    [data, height, width, hasEnoughData]
  )
  ...
  const animationKey = `${replayKey}-${effectiveDuration}`
    ...
    >
      <defs>
        ...
        <clipPath id={`${sparklineId}-clip`}>
          <rect height={clipSize.height} key={animationKey} width={effectiveDuration > 0 ? 0 : clipSize.width} x={clipSize.x} y={clipSize.y}>
            {effectiveDuration > 0 ? (
              <animate
                attributeName="width"
                calcMode="spline"
      ...
      {showEndpoint && lastPoint ? (
        <circle
          ...
          opacity={effectiveDuration > 0 ? '0' : '1'}
          r={strokeWidth + 1}
        >
          {effectiveDuration > 0 ? (
            <animate attributeName="opacity" dur={`${effectiveDuration}ms`} fill="freeze" keyTimes="0;0.78;1" values="0;0;1" />
          ) : null}
        </circle>
      ) : null}
    </svg>
